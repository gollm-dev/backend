...
async def initiate_model_download(model: ModelDownloadRequest):
    model_obj = await db.models_statuses.find_one({"model_name": model.model_name})
    ...
    return JSONResponse(content={"message": f"{model.model_name} downloading started."}, status_code=202)
...
async def create_completion(request: CompletionRequest):
    try:
        model_obj = await db.models_statuses.find_one({"model_name": request.model_name})
